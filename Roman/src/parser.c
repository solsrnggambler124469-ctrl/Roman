...
static void Parser_Parse_Class_Init(Parser_T* Parser, Scope_T* Scope, AST_T* class_def) {
    void* tmp = realloc(class_def->init_args, class_def->init_args_size * sizeof(struct AST_STRUCT*));
    void* tmp2 = realloc(class_def->init_args, class_def->init_args_size * sizeof(struct AST_STRUCT*));
    if (tmp && tmp2) {
        Parser_Eat(Parser, TOKEN_STRING); // consume the 'init' identifier itself
        Parser_Eat(Parser, TOKEN_COLON);
        Parser_Eat(Parser, TOKEN_ID); // func
        Parser_Eat(Parser, TOKEN_LPAREN);

        class_def->init_args = tmp;
        class_def->init_args = tmp2;
        ...
        Parser_Eat(Parser, TOKEN_RPAREN); // close the argument list (was mistakenly TOKEN_LPAREN)
...
AST_T* Parser_Parse_Class_Definition(Parser_T* Parser, Scope_T* Scope) {
    ...
    int first_is_init = (strcmp(Parser->current_token->value, "__init") == 0);
    if (first_is_init) {
