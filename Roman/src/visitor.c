...
AST_T* VV_For(Visitor_T* visitor, AST_T* node) {
    For_Tuple* ttt = get_for_data(visitor, node);

    if (ttt->for_var2->type != AST_NOOP && ttt->for_end_do->type == AST_STRING) {
        //for_var->scope = node->for_body->scope;

        bool get_res() {
            ttt->condition_result = Visitor_Visit(visitor, node->for_condition);
        ...
        if (strcmp(ttt->for_end_do->string_value, "++") == 0) {
            minus = false;
        } else if (strcmp(ttt->for_end_do->string_value, "--") == 0) {
            minus = true;
        };

        if (minus) {
            for (int i=(int)ttt->for_var2->number_value;get_res() == true;i--) {
                last_var = Visitor_Visit(visitor, node->for_body);
                ttt->for_var2->number_value = i;
            };
        } else {
            for (int i=(int)ttt->for_var2->number_value;get_res() == true;i++) {
                last_var = Visitor_Visit(visitor, node->for_body);
...
AST_T* VV_Arrow(Visitor_T* visitor, AST_T* node) {
    ...
    if (left->type == AST_DICTIONARY_DEFINITION) {
        ...
        // NOTE: "> \"init\"" runs the constructor body, but there is currently no syntax
        // for passing arguments through '>' — init_args is parsed and stored, but nothing
        // binds values to it here. Any parameter the constructor body references will
        // fail with "undefined variable" unless it's also a scope binding some other way.
        // A real call form (e.g. `ClassName > init(1, 2)`) would need its own grammar and
        // dispatch, similar to VV_Function_Call, rather than being folded into VV_Arrow.
